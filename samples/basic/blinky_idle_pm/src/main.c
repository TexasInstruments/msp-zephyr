...
const struct gpio_dt_spec debug_pin = GPIO_DT_SPEC_GET(DT_NODELABEL(debug_pin), gpios);
const struct gpio_dt_spec debug_pin_1 = GPIO_DT_SPEC_GET(DT_NODELABEL(debug_pin_1), gpios);
...
int main(void)
{
	...
	gpio_pin_configure_dt(&debug_pin_1, GPIO_OUTPUT_ACTIVE);
	if (ret < 0) {
		return 0;
	}
