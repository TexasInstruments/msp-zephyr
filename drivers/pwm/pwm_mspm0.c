...
static void mspm0_setup_pwm_out(const struct pwm_mspm0_config *config, struct pwm_mspm0_data *data)
{
	...
	for (i = 0; i < config->cc_idx_cnt; i++) {
		DL_Timer_setCaptureCompareValue(config->base, data->pulse_cycle[i],
						config->cc_idx[i]);
				...
				uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	DL_Timer_setCaptureCompareValue(config->base, data->pulse_cycle[channel],
				    ...
				    struct pwm_mspm0_data *data)
{
	...
	DL_Timer_setCaptureCompareInput(
		config->base, 0,
		((config->cc_idx[0] & 0x1) ? DL_TIMER_CC_IN_SEL_CCPX : DL_TIMER_CC_IN_SEL_CCP0),
		config->cc_idx[0]);

	DL_Timer_setCaptureCompareInput(config->base, 0, GPTIMER_IFCTL_01_ISEL_CCPX_INPUT_PAIR,
					(config->cc_idx[0] ^ 1));

	DL_Timer_setCaptureCompareCtl(config->base, DL_TIMER_CC_MODE_CAPTURE,
				      DL_TIMER_CC_CCOND_TRIG_FALL, config->cc_idx[0]);

	DL_Timer_setCaptureCompareCtl(config->base, DL_TIMER_CC_MODE_CAPTURE,
				      DL_TIMER_CC_CCOND_TRIG_RISE, (config->cc_idx[0] ^ 1));

	DL_Timer_setCCPDirection(config->base, DL_TIMER_CC0_INPUT);

	DL_Timer_setCounterControl(config->base, DL_TIMER_CZC_CCCTL0_ZCOND,
...
static void mspm0_cc_isr(const struct device *dev)
{
	...
	if (data->flags & PWM_CAPTURE_TYPE_PERIOD) {
		cc1 = DL_Timer_getCaptureCompareValue(config->base, config->cc_idx[0] ^ 0x1);
	}

	/* ignore the unsynced counter value for pwm mode */
	if (data->is_synced == false && data->cmode != CMODE_EDGE_TIME) {
	...
	if (data->flags & PWM_CAPTURE_TYPE_PULSE || data->cmode == CMODE_EDGE_TIME) {
		cc0 = DL_Timer_getCaptureCompareValue(config->base, config->cc_idx[0]);
	}
...
#define MSPM0_PWM_DATA(n) .out_mode = MSPM0_PWM_MODE(DT_STRING_TOKEN(DT_DRV_INST(n), ti_pwm_mode)),
...
#define PWM_DEVICE_INIT_MSPM0(n)                                                                   \
	...
	static const struct pwm_mspm0_config pwm_mspm0_config_##n = {                              \
		...
		.cc_idx = {DT_INST_FOREACH_PROP_ELEM(n, ti_cc_index, MSPM0_CC_IDX_ARRAY)},         \
