	...
	while (channels) {
		...
		if (mem_ctl_count < config->max_result) {
			DL_ADC12_configConversionMem(config->base, mem_ctl_count,
						     (ch << ADC12_MEMCTL_CHANSEL_OFS), vrsel, stime,
...
#define ADC_DT_CLOCK_SOURCE(x) DT_INST_ENUM_IDX(x, ti_clk_source)
...
#define ADC_DT_CLOCK_RANGE(x) DT_INST_ENUM_IDX(x, ti_clk_range)
...
#define MSPM0_ADC_INIT(index)                                                                      \
	...
	static const struct adc_mspm0_cfg adc_mspm0_cfg_##index = {                                \
		...
		.clock_config = {.clockSel = ADC_DT_CLOCK_SOURCE(index),                           \
				 .freqRange = ADC_DT_CLOCK_RANGE(index),                           \
				 .divideRatio = ADC_DT_CLOCK_DIV(index)},                          \
		.max_result = DT_INST_PROP(index, max_result),                                     \
